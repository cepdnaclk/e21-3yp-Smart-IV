...
// --------------------------- IV configuration -------------------------------
static constexpr float DEFAULT_TARGET_MLHR = 0.0f;   // nurse/user must enter with keypad: 80#
static constexpr float DEFAULT_BAG_VOLUME_ML = 0.0f; // nurse/user must enter with keypad: 500D
...
struct SystemState {
  ...
  float maxVolume = DEFAULT_BAG_VOLUME_ML;
  float volRemaining = DEFAULT_BAG_VOLUME_ML;
...
bool calibrateIrSensor() {
  ...
  noInterrupts();
  lastIrEdgeUs = micros();
  lastAcceptedDropUs = 0;
  interrupts();

  // FALLING is typical for LM393 when beam is interrupted, but use RISING if your module is inverted.
  // If calibration says noisy, DO NOT attach the interrupt. This prevents false volume drain.
  if (ok) {
    attachInterrupt(digitalPinToInterrupt(PIN_IR), onIrEdge, FALLING);
  } else {
    Serial.println("[IR] Interrupt disabled until you fix alignment/noise and press C to recalibrate.");
  }
  return ok;
}
...
void resetSessionCounters() {
  ...
  state.volRemaining = state.maxVolume;
...
void startSession() {
  xSemaphoreTake(stateMutex, portMAX_DELAY);
  bool targetOk = state.targetMlhr > 0.1f;
  bool volumeOk = state.maxVolume > 0.1f;
  bool irOk = state.sensorReady && !state.sensorNoisy;
  xSemaphoreGive(stateMutex);

  if (!targetOk || !volumeOk || !irOk) {
    Serial.print("[SESSION] Start refused: ");
    if (!targetOk) Serial.print("target not set; ");
    if (!volumeOk) Serial.print("bag volume not set; ");
    if (!irOk) Serial.print("IR not ready/noisy; ");
    Serial.println();
    return;
  }
...
void sensorTask(void *pv) {
  ...
  for (;;) {
    ...
    // Do not show/control/count flow before a real session has been started.
    // Also ignore IR completely if calibration marked it noisy.
    if (!state.sessionActive || !state.sensorReady || state.sensorNoisy) {
      dpm = 0.0f;
      mlhr = 0.0f;
    }
    ...
    if (state.sessionActive && state.sensorReady && !state.sensorNoisy) {
      ...
      state.volRemaining = max(0.0f, state.maxVolume - ((float)sessionDrops / (float)DROP_FACTOR));

      bool startupGrace = (millis() - state.sessionStartMs) < 30000UL; // 30 s grace after pressing A

      if (state.volRemaining <= EMPTY_VOLUME_ML) {
        state.status = STATUS_EMPTY_BAG;
      } else if (!startupGrace && noFlowTimeout && state.volRemaining > EMPTY_VOLUME_ML) {
...
void commTask(void *pv) {
  for (;;) {
    ...
    doc["maxVolume"] = (int)snapshot.maxVolume;
    ...
    // ESP-NOW payload limit is 250 bytes. Send the actual JSON length only,
    // not a fixed 256-byte struct and not a padded buffer.
    esp_err_t result = esp_now_send(RECEIVER_MAC, reinterpret_cast<uint8_t *>(json), len);
  ...
  for (;;) {
    ...
    if (key) {
      if (key >= '0' && key <= '9') {
        ...
        int val = keypadBuffer.toInt();
        if (keypadBuffer.length() && val >= 10 && val <= 2000) {
          xSemaphoreTake(stateMutex, portMAX_DELAY);
          state.maxVolume = (float)val;
          state.volRemaining = (float)val;
          state.sessionDrops = 0;
          xSemaphoreGive(stateMutex);
          Serial.printf("[KEYPAD] Bag volume set to %d mL\n", val);
        } else {
          resetSessionCounters();
    ...
    if (lcdOk) {
      ...
      lcd.print(BED_ID);
      lcd.print(" ");
      lcd.print(snapshot.sessionActive ? statusToString(snapshot.status) : "SETUP");
...
void setup() {
  ...
  Serial.println("[KEYPAD] digits+#=target, digits+D=bag volume, A=start, B=stop/clamp, C=calibrate IR, D=reset counters");
  Serial.println("[SETUP] Example: press 80# then 500D then A. Start is refused until target, volume, and IR are ready.");
}
